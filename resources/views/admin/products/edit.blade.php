            ...
            <form method="post" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-6">
                    <label class="block" for="name">
                        ...
                        <input type="text" class="mt-1 block w-full" id="name" name="name" value="{{ old('name', $product->name) }}" />
                    </label>
                    <label class="block" for="description">
                        <span class="text-gray-700">{{ __('Product Description') }}</span>
                        <textarea class="mt-1 block w-full" id="description" name="description">{{ old('description', $product->description) }}</textarea>
                    </label>
                    ...
                    <label class="block" for="manufacturer_id">
                        ...
                        <select class="mt-1 block w-full" id="manufacturer_id" name="manufacturer_id">
                            @foreach($manufacturers as $manufacturer)
                                <option value="{{ $manufacturer->id }}" {{ $manufacturer->id === old('manufacturer_id', $product->manufacturer_id) ? 'selected' : '' }}>{{ $manufacturer->name }}</option>
                            @endforeach
                        </select>
                    </label>
                    ...
                    <label class="block" for="size_ids">
                        ...
                        <select class="mt-1 block w-full" id="size_ids" name="size_ids[]" multiple>
                            @foreach($sizes as $size)
                                <option value="{{ $size->id }}" {{ in_array($size->id, old('size_ids', $product->sizes->pluck('id')->toArray())) ? 'selected' : '' }}>{{ $size->name }}</option>
                            @endforeach
                        </select>
                    </label>
